...
struct FeedView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            // Load breaking news for carousel
            if viewModel.breakingNews.isEmpty {
                await viewModel.loadBreakingNews(apiService: apiService)
            }
                    ...
                    ScrollViewReader { proxy in
                        ScrollView {
                            LazyVStack(spacing: 16) {
                                ...
                                // Breaking News Carousel (inside ScrollView for better space utilization)
                                if !viewModel.breakingNews.isEmpty {
                                    BreakingNewsCarousel(
                                        stories: viewModel.breakingNews,
                                        onStoryTap: { story in
                                            selectedStory = story
                                        }
                                    )
                        ...
                        .onChange(of: viewModel.shouldScrollToTop) { _, shouldScroll in
                            if shouldScroll {
                                ...
                                // PERFORMANCE FIX: Added [weak viewModel] to prevent retain cycle
                        ...
                        .onChange(of: notificationStoryId) { _, storyId in
                            if let storyId = storyId {
                                ...
                                // Scroll to the story
                                // PERFORMANCE FIX: Added weak capture to prevent retain cycle
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                                    withAnimation(.easeOut(duration: 0.5)) {
                                        proxy.scrollTo(storyId, anchor: .center)
                                    }
                                    // Reset after scroll
                                    // Note: notificationStoryId is captured by value (UUID), so weak not needed here
...
class FeedViewModel: ObservableObject {
    ...
    @Published var breakingNews: [Story] = []
    ...
    func loadStories(apiService: APIService, refresh: Bool = false) async {
        ...
        // But also trigger a refresh to get latest content
        if !hasLoadedCache && !refresh && stories.isEmpty {
            loadFromCache()
            hasLoadedCache = true
            // Trigger background refresh to get latest content
            Task {
        ...
        do {
            ...
            // Update feed: Replace on refresh OR first page (page 0), append only when paginating
            // Trust server-side ordering - API already returns stories in optimal display order
            if refresh || currentPage == 0 {
                // REPLACE cached stories with fresh API content on first load or refresh
                stories = newStories
                log.log("✅ Feed replaced with \(stories.count) fresh stories for category: \(selectedCategory?.displayName ?? "all")", category: .ui, level: .info)
            } else {
                // Append for pagination (page > 0), remove duplicates
                let existingIds = Set(stories.map { $0.id })
                let uniqueNewStories = newStories.filter { !existingIds.contains($0.id) }
                stories = stories + uniqueNewStories
            }
            currentPage += 1
            lastFetchDate = Date()
    ...
    /// Load breaking news stories for carousel
    func loadBreakingNews(apiService: APIService) async {
        do {
            log.log("📰 Loading breaking news", category: .ui, level: .info)
            let news = try await apiService.getBreakingNews()
            // Limit to 5 most recent breaking news items for carousel
            self.breakingNews = Array(news.prefix(5))
            log.log("✅ Loaded \(self.breakingNews.count) breaking news items", category: .ui, level: .info)
        } catch {
            log.logError(error, context: "Failed to load breaking news")
        }
    }
    ...
    func startPolling(apiService: APIService) {
        ...
        // PERFORMANCE FIX: Increased polling interval from 5 to 10 minutes
        // Reduces network activity and battery drain by 50%
        pollingTimer = Task {
            while !Task.isCancelled {
                // Wait 10 minutes between polls for new stories
    ...
    func loadPendingNewStories() {
        ...
        // Smoothly merge pending stories - prepend new stories (server already ordered them)
        withAnimation(.spring(response: 0.5, dampingFraction: 0.75)) {
            stories = pendingNewStories + stories
        }
...
fileprivate func timeAgo(from date: Date) -> String {
    let now = Date()
...
struct BreakingNewsCarousel: View {
    let stories: [Story]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 6) {
                ...
                Text("LIVE")
                    .font(.outfit(size: 10, weight: .bold))
                    .foregroundStyle(.red)
                    .tracking(0.5)
                Spacer()
