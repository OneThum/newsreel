        ...
        for (index, line) in lines.enumerated() {
            ...
            if !trimmed.isEmpty {
                ...
                if trimmed.hasPrefix("**") && trimmed.contains("**") {
                    startIndex = index + 1
                    // Skip the blank line after headline if present
                    if startIndex < lines.count && lines[startIndex].trimmingCharacters(in: .whitespaces).isEmpty {
                        startIndex += 1
                    }
                }
                break
